...
submit_puzzle:
    ...
    li      $t2, 0
wait_paint_buckets:
    bne     $t2, $0, start_paint                # if paint_buckets != 0, then start paint
    lw      $t2, GET_PAINT_BUCKETS($0)          # $t2 = paint_bucktes
    j		wait_paint_buckets				    # jump to target
...
start_paint:
    ...
    li      $t7, 40
loop:
    lw      $t2, GET_PAINT_BUCKETS($0)          # get # of paint buckets left
    blt     $t2, $t7, request_puzzle            # request new puzzle if paint backets less than __
...
bonk_interrupt:
    ...
    li      $t2, 230                        # $t2 = 230
    sw      $t2, ANGLE($0)                  # relative angle = 230 (turn down)
    li      $t2, 10                         # $t2 = 10
